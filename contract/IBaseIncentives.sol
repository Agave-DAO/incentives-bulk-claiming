...
interface IBaseIncentives {
    ...
    function bulkClaimRewardsOnBehalf(address[] memory assets, uint256 amount, address user, address to)
        external
        returns (uint256);
    ...
    function configureAssets(
        ...
        uint256[] memory assetDecimals
    ) external;
    function disableAssets(address[] memory assets) external;
    function getAssetData(address asset) external view returns (uint256, uint256, uint8, uint256, bool);
    ...
    function setRewardTokenAdjustment(bool RewardTokenAdjustmentMultiplier, uint256 RewardTokenAdjustmentAmount)
        external;
