...
async function looper() {
  ...
  fs.writeFile(
    "users.json",
    `{\n\"users":${JSON.stringify(unclaimedUsers)}\n,\"amounts":${JSON.stringify(unclaimedAmounts)}\n}`,
    {
      encoding: "utf8",
      flag: "w",
      mode: 0o666,
...
async function getUsersWithRewards(users) {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    if (tempUsers.length >= multicallSize || (i + 1 >= users.length && tempUsers.length > 0)) {
  ...
  for (let i = 0; i < users.length; i++) {
    if (output[i]["result"]) {
      ...
      totalAmount = totalAmount + output[i]["result"];
    }
  }
